...
def get_weather():
    ...
    weather_data = read_local_data("weather")
    for entry in weather_data:
        login[entry] = weather_data[entry]
    ...
    for field in DAILY_FIELDS:
        login["fields"].append(field)

    data = (requests.request("GET", URL, params=login)).json()
    ...
    save_data(hourly_weather_info, daily_weather_info)
...
# TODO: This is the formatting for current_time()
#(datetime.now()).strftime("%-I:%M %p")

def weather_handler(input_queue, output_queue):
    ...
    if not json_object or verify_time(json_object["timeIntervals"][0]["hour"][0]["startTime"]):
...
# Consider merging into antoher function in the future
...
# Consider merging into another function in the future
