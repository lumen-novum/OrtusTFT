...
# I think putting all of the screens into a single class is a much better idea instead of
# creating a separate init function that forces the use of global variables.
# However, I have a feeling that this still isn't the best way I could've implemented this.
# If you have an idea of how this could be better, please leave a PR or an issue.

class ScreenCollection:
    def __init__(self, display_q, touch_q, button_q, weather_q, d):
        self.display_command = display_q
        self.weather_queues = weather_q
        self.touch_queue = touch_q
        self.button_queue = button_q
        self.demo = d
    ...
    def background_process(self, current_screen, btn_keys, **kwargs):
        ...
        while True:
            ...
            new_data = weather.request_weather_info(self.weather_queues, "day_phase", wait=True)

            if new_data:
                if current_screen == "Home":
                    day_phase = weather.get_day_phase(weather.request_weather_info(self.weather_queues, "day_phase"))
                    interface.background(self.display_command, day_phase)
                    local = self.home_formatting(day_phase)

                    
                    interface.update_element(self.display_command, "welcome_label", "text", local[0])
                    interface.update_element(self.display_command, "weather_button", "image", "weather_icons/{}.png".format(new_data[local[1]]))
                    current_temperature = new_data["temperature"]
                day_phase = weather.get_day_phase(new_data)
                interface.background(self.display_command, day_phase)
            
            if current_screen == "Home":
                interface.update_element(self.display_command, "time_temp_label", "text", "{} F°   {}".format(kwargs["current_temperature"], weather.current_time()))
            sleep(0.1)

    def clean_up(self):
        interface.clear_screen(self.display_command, self.button_queue)
    ...
    def home(self):
        ...
        self.NAME_label = interface.format_text("self.NAME", self.NAME, "midtop", (120,40), "header")
        ...
        interface.create_element(self.display_command, welcome_label, self.NAME_label, time_temp_label, weather_button, calendar_button, gallery_button, about_button, button_reg=self.button_queue)
        interface.background(self.display_command, day_phase)
        ...
        return self.background_process("Home", btn_keys, current_temperature=current_temperature)

    def weather_report(self, page):
        if page == 1:
            weather_info = weather.request_weather_info(self.weather_queues, "stats")
            ...
            interface.create_element(self.display_command, back_button, time_label, next_page, button_reg=self.button_queue)
            ...
            interface.create_element(self.display_command,
            ...
            return self.background_process("Stats", btn_keys)

        elif page == 2:
            weather_info = weather.request_weather_info(self.weather_queues, "stats 2")

            back_button = interface.format_label("back_button", ((5, 5), interface.MINI_ICON), "topleft", image="previous.png", button=True)
            time_label = interface.format_text("time_label", "", "midtop", (120, 5), "subheader")

            interface.create_element(self.display_command, back_button, time_label, button_reg=self.button_queue)
            ...
            return self.background_process("Stats 2", btn_keys)
        else:
    ...
    def about(self):
        back_button = interface.format_label("back_button", ((5, 5), interface.MINI_ICON), "topleft", image="previous.png", button=True)
        interface.create_element(self.display_command, back_button, button_reg=self.button_queue)
        ...
        username = interface.format_text("username", "lumen-novum", "midtop", (120,105), "subheader")
        ...
        interface.create_element(self.display_command, title, author, username, qr_code, side_note, version)
        ...
        return self.background_process("About", btn_keys)

    def sleep_screen(self, last_screen):
        interface.create_element(self.display_command, interface.format_label("power", ((0, 0), (240, 320)), "topleft", fill=interface.BLACK, button=True), button_reg=self.button_queue)
        if not self.demo:
            with open("/sys/class/backlight/soc:backlight/brightness", "w") as backlight:
                backlight.write("0")
        return self.background_process("sleep_screen", {"power": last_screen})

    def wip(self):
        def cone_row(height):
            for cone_num in range(7):
                cone_pos = (cone_num * 32) + 8
                interface.create_element(self.display_command, interface.format_label(str(cone_num) + str(height), ((cone_pos, height), interface.MINI_ICON), "topleft", image="cone.png"))
        ...
        interface.create_element(self.display_command, title, desc1)
        ...
        return self.background_process("WIP", btn_keys)