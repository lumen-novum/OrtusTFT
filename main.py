...
def main():
    ...
    sc = screens.ScreenCollection(display_command, touch_queue, button_queue, (weather_input, weather_output), demo)
    ...
    while True:
        if current_screen == "Home":
            current_screen = sc.home()
        elif current_screen == "Stats":
            current_screen = sc.weather_report(1)
        elif current_screen == "Stats 2":
            current_screen = sc.weather_report(2)
        elif current_screen == "About":
            current_screen = sc.about()
        elif current_screen[0:3] == "Off":
            current_screen = sc.sleep_screen(current_screen[4:len(current_screen)])
        else:
            current_screen = sc.wip()
